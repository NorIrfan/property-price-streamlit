...
def train_models(df):
    ...
    model_defs = {
        ...
        "Random Forest (Small)": RandomForestRegressor(
            n_estimators=30, max_depth=12, min_samples_leaf=20,
...
if menu == "Overview":
    st.write("Regression task: predict **Transaction Price** based on income, population, and property attributes.")
    ...
    if "income" in df.columns:
        st.plotly_chart(px.scatter(df, x="income", y=TARGET_COL, title="Income vs Transaction Price"), use_container_width=True)
    if "population" in df.columns:
        st.plotly_chart(px.scatter(df, x="population", y=TARGET_COL, title="Population vs Transaction Price"), use_container_width=True)
    ...
    st.plotly_chart(px.bar(res_df, x="Model", y="RMSE", title="RMSE Comparison (lower is better)"), use_container_width=True)
    ...
    for col in feature_cols:
        if col in ["Property_Type", "Tenure", "District", "state"]:
            user_input[col] = st.text_input(col, value="")
        elif col == "Month_Year":
            user_input[col] = st.text_input(col, value="2024-01-01")
        else:
